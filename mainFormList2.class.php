 ...
 * Date : July 13, 2009
 * Filename : mainFormList2.class.php
...
class mainFormList
{
	...
	public function mainFormList($table, $start = NULL, $numRecs = NULL) // $lim Stands for limit 
	{
		...
		$this->queryString = "SELECT transNum, date, transDescript, custID FROM ".$table." ORDER BY year DESC, transNum DESC".$this->limit;
	...
	{
		$this->queryString = "SELECT transNum, date, transDescript, custID FROM ".$this->tableName." 
	...
	public function displayRecords($search = NULL)
	{
		...
		if(mysql_num_rows($query) > 0)
		{
			...
			echo "<th>Customer</th>$xtraTh<th>EDIT</th><th>DELETE</th></tr>";
			while(list($num, $date, $transDesc, $custID) = mysql_fetch_array($query))
			{
				...
				echo "<td>".strtoupper($custID)."</td>$xtraTd<td><a href=edit".strtoupper($this->tableName).".php?num=$num>[EDIT]</a></td>
